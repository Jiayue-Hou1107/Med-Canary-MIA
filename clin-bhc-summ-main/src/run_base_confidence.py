...
from transformers import AutoModelForCausalLM, AutoTokenizer, BertTokenizer, BertModel, BertConfig
...
def load_base_model(args):
    base_model_path = constants.MODELS.get(args.model)
    
    if base_model_path is None or not os.path.exists(base_model_path):
        raise FileNotFoundError(f"在 constants.py 中找不到模型路径: {args.model}")
    
    print(f'=== Loading the base model: {base_model_path} ===')
    
    # Specialized Tokenizer for Llama/Qwen
    ...
    tokenizer.padding_side = "left" # Inference must be left-padded.
    
    # Dual-GPU
    print(f"Loading {args.model} by unsing dual-GPU...")
...
def run_base_subset_with_probs(args, model, tokenizer, task_name, num_samples):
    print(f"\n>>> Running Base Inference on {task_name.upper()} (Sampling {num_samples})")
    ...
    for i in tqdm(range(0, len(data_list), args.batch_size), desc=f"{args.model}"):
...
def main():
    ...
    try:
        ...
        # Running sampling on the training set and test set
        train_records = run_base_subset_with_probs(args, model, tokenizer, task_name='trn', num_samples=100)
        test_records = run_base_subset_with_probs(args, model, tokenizer, task_name='test', num_samples=100)

        all_records = train_records + test_records
        df = pd.DataFrame(all_records)
        
        # Save Results
        save_path = os.path.join(args.dir_out, f"base_original_results")
        os.makedirs(save_path, exist_ok=True)
        csv_file = os.path.join(save_path, f"{args.model}_base_confidence.csv")
        df.to_csv(csv_file, index=False, encoding='utf-8-sig')
        print(f"\nSaved Results to: {csv_file}")

        # Indicator Summary Printout
        def report(records, name):
            if not records: return
            print(f"\n" + "="*20 + f" {name} " + "="*20)
            m = compute_metrics([r['reference'] for r in records], [r['prediction'] for r in records])
            for k, v in m.items(): print(f"{k:12}: {v:.2f}")

        report(train_records, "TRAIN (100 samples)")
        report(test_records, "TEST (100 samples)")
