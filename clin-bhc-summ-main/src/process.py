...
from transformers import AutoModelForSeq2SeqLM, AutoModelForCausalLM, AutoTokenizer, default_data_collator, AutoTokenizer
import constants
import parser
...
def load_data(args, task):
    ...
    with open (fn_finding, 'r') as f_f, open(fn_summary, 'r') as f_s:
        ...
        for idx, finding in enumerate(lines_finding):
            ...
            elem_summary = lines_summary[idx].rstrip()
...
def causal_formatting_func(sample, instruction):
    ...
    # Medical tasks (Summrization) prompt
    instruction_text = f"### Instruction:\n{instruction}"
    context = f"### Clinical Findings:\n{sample['sentence']}" if len(sample["sentence"]) > 0 else None
    response = f"### Clinical Impression:\n{sample['text_label']}"
    
    # Use the standard format for medical documents
    prompt = "\n\n".join([i for i in [instruction_text, context, response] if i is not None])
    return prompt
...
def get_in_context_prompt(list_finding, list_summary, task, args, knn=True):
    ...
    for i, element in tqdm(enumerate(list_finding), total=len(list_finding)):
        ...
        if args.model[0:6] == "llama2" or args.model[0:6] == "falcon":
            new_element = '\n'.join([causal_formatting_func({'sentence': trn_findings[j], 'text_label': trn_summaries[j]}, constants.START_PREFIX) for j in
                                     example_idcs])
            new_element += '\n' + causal_formatting_test({'sentence': list_finding[i]}, constants.START_PREFIX)
        else:
...
def preprocess_function(examples, tokenizer, model_name):
    """
    Process input data, convert it into token IDs readable by the model, and set labels for loss calculation.
    Supports Llama 2 (old format) and Llama 3 (new format).
    """
    inputs = []
    targets = [] 
    
    is_llama3 = "llama3" in model_name.lower()
    is_qwen = "qwen" in model_name.lower()

    # Iterate through each data entry to construct the text.
    for i in range(len(examples['sentence'])):
        instruction_content = constants.START_PREFIX  # System Prompt
        input_content = examples['sentence'][i]       # User Input
        response_content = examples['text_label'][i]  # Target Output
        
        if is_qwen:
            prompt_part = f"<|im_start|>system\n{instruction_content}<|im_end|>\n"
            prompt_part += f"<|im_start|>user\n### Clinical Findings:\n{input_content}\n\nProvide the Clinical Impression.<|im_end|>\n"
            prompt_part += f"<|im_start|>assistant\n"
            full_text = f"{prompt_part}{response_content}<|im_end|>"
            ...
            # System 
            sys_part = f"<|start_header_id|>system<|end_header_id|>\n\n{instruction_content}<|eot_id|>"
            
            # User
            user_text_formatted = f"### Clinical Findings:\n{input_content}\n\nBased on the findings above, provide the Clinical Impression."
            user_part = f"<|start_header_id|>user<|end_header_id|>\n\n{user_text_formatted}<|eot_id|>"
            
            # Assistant
            assist_header = f"<|start_header_id|>assistant<|end_header_id|>\n\n"
            
            # Combine into a prompt
            prompt_part = sys_part + user_part + assist_header
            
            # Combine into Full Text (Prompt + Answer + end note)
            full_text = f"{prompt_part}{response_content}<|eot_id|>"

        # Llama 2 
        else:
            instruction_text = f"### Instruction:\n{instruction_content}"
            context = f"### Clinical Findings:\n{input_content}"
            response_header = "### Clinical Impression:\n"
            
            prompt_part = f"{instruction_text}\n\n{context}\n\n{response_header}"
            
            full_text = f"{prompt_part} {response_content}{tokenizer.eos_token}"
        
        inputs.append(full_text)
        targets.append(prompt_part)
...
def get_loader_distributed(dataset, tokenizer, batch_size, model, sampler):
    ''' given list of input and target texts, return dataloader for distributed training '''
...
def debug_labels(dataset, tokenizer, model_name, num_samples=1):
    """
    Debugging Function: Verify label settings are correct
    """
    print(f"\n=== Debugging Tag Settings (Model: {model_name}) ===")
    
    # Process a small portion of data for debugging purposes
    debug_data = dataset.select(range(min(num_samples, len(dataset))))
    
    processed = debug_data.map(
    ...
    for i in range(len(processed)):
        input_ids = processed[i]["input_ids"]
        labels = processed[i]["labels"]
        
        print(f"\n--- Sample {i} ---")
        
        # Decode the complete sequence
        full_text = tokenizer.decode(input_ids, skip_special_tokens=False)
        print(f"complete sequence: {full_text[:200]}..." if len(full_text) > 200 else f"complete sequence: {full_text}")
        
        # Statistical Masking Situation
        non_masked = (labels != -100).sum().item()
        total = (input_ids != tokenizer.pad_token_id).sum().item()
        mask_ratio = non_masked / total if total > 0 else 0
        
        print(f"Tag Statistics: {non_masked}/{total} tokens loss ({mask_ratio*100:.1f}%)")
        
        answer_tokens = []
        for j, (token_id, label_id) in enumerate(zip(input_ids, labels)):
            if label_id != -100 and token_id != tokenizer.pad_token_id:
                token = tokenizer.decode([token_id])
                answer_tokens.append(token)
        
        answer_text = "".join(answer_tokens)
        print(f"answer part: {answer_text}")
